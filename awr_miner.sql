define AWR_MINER_VER = 25.1.3

/*
	Version History

	2024-06-16	24.7.1 	mgr	Fixed SIZE-ON-DISK when CDB has PDBs with different block size
...
begin
    ...
    -- Oracle 12c, specifically the 12.1 version, introduced the multitenant architecture
	--  We need to make sure that is running in CDB$ROOT
	if :DB_VERSION_1 >= 12.1 then
        SELECT SYS_CONTEXT('USERENV', 'CON_ID') INTO l_con_id FROM dual;
        IF l_con_id > 1 THEN
            DBMS_OUTPUT.PUT_LINE('Error: This script must be executed from CDB$ROOT right now is running on CON_ID ->  ' || l_con_id);
            RAISE_APPLICATION_ERROR(-20001, 'Not in CDB$ROOT. Script execution aborted.');
        END IF;
	end if;
...
begin
	if :DB_VERSION_1 <= 11.2 then
		:DB_SIZE_QUERY_1 := q'! ts_info as (
			...
			select s.snap_id,
                   round(sum(sp.tablespace_size * ts.block_size)/1024/1024/1024, 2) size_gb,
                   round(sum(sp.tablespace_usedsize * ts.block_size)/1024/1024/1024, 2) used_size_gb
